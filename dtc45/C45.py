...
class C45:
    ...
    def fit(self):
        ...
        self.fitInternal(self.dataset, 0, None, None, None)
    ...
    def fitInternal(self, currentData, parent_id, parent_edge, parent_edge_operator, parent_data_type):
        ...
        if (len(curr_target) == 1):
            self.tree.append([cur_idx, parent_id, None, None, 0.0, 0.0, parent_edge, parent_edge_operator, 0.0, curr_target[0], targetCompostion, None, parent_data_type])
            print("......  Fitting " + str(self.cur_num_node) + " nd node, parent id : " + str(parent_id) + ", parent edge :" + str(parent_edge) + " ...... Done!!!\n")
            return
        ...
        if (currentData.shape[1] < 2):
            ...
            self.tree.append([cur_idx, parent_id, None, None, 0.0, 0.0, parent_edge, parent_edge_operator, 0.0, selected_class, targetCompostion, None, parent_data_type])
            print("......  Fitting " + str(self.cur_num_node) + " nd node, parent id : " + str(parent_id) + ", parent edge :" + str(parent_edge) + " ...... Done!!!\n")
            return
        ###### Check terminate condition

        ## select a feature as current node
        splitter = FeatureSelector(self.pool_size, self.attributes_info, currentData, curr_target)
        splitter.doSelect()
        ...
        if (cur_threshold != None and len(cur_threshold) == 1): # if feature is numeric feature. Assuming feature that has one unique edge is numeric feature.
            ...
            self.fitInternal(XXY_new2, cur_idx, cur_threshold, '<=', cur_feature_type)
            ...
            max_count, selected_class = self.selectMajorityClass(curr_target, curr_target_count)
            self.tree.append(
                [cur_idx, parent_id, None, None, 0.0, 0.0, parent_edge, parent_edge_operator, 0.0, selected_class, targetCompostion, None,
                 parent_data_type])
            return
        ...
        self.tree.append(
            [cur_idx, parent_id, cur_attr_name, col_idx, cur_gain,
             cur_split_info, parent_edge, parent_edge_operator, cur_threshold, None, targetCompostion, thresholdCompostion, cur_feature_type])
